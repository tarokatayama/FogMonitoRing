...
processed2 <- All_Sites_Processed %>%
  mutate(Station = as_factor(Station))

processed2$Station <- as.factor(processed2$Station)

ggplot(data = processed2) +
  geom_ridgeline(aes(x = TIMESTAMP, y = Station, height = (LWmV_66ft_Avg - min(LWmV_66ft_Avg)),  fill = Station)) + 
  #facet_wrap(~day)+
  scale_y_discrete()+
